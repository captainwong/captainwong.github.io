...
执行`vagrant halt`后，winnfsd仍然在运行，有时导致`rm: cannot remove '<FILENAME>': Input/output error`错误，参考[https://github.com/winnfsd/vagrant-winnfsd/issues/105](https://github.com/winnfsd/vagrant-winnfsd/issues/105)，停止虚拟机后手动杀一下winnfsd.exe进程即可。
...
4. `require guzzlehttp/guzzle:~6.0` [Guzzle PHP HTTP Client](https://guzzle.readthedocs.io/en/latest/)
